...
class GembaPay_Webhook {
    ...
    private static function handle_payment_completed($order, $payment) {
        ...
        // Build order note
        $note = __('GembaPay payment completed.', 'gembapay-woocommerce');
        
        if (isset($payment['network'])) {
            $network = ucfirst($payment['network']);
            $note .= ' ' . sprintf(__('Network: %s.', 'gembapay-woocommerce'), $network);
        }

        if (isset($payment['txHash'])) {
            $note .= ' ' . sprintf(__('TX: %s', 'gembapay-woocommerce'), substr($payment['txHash'], 0, 20) . '...');
        }

        if (isset($payment['usdAmount'])) {
            $note .= ' ' . sprintf(__('Amount: $%s USD', 'gembapay-woocommerce'), $payment['usdAmount']);
        }
    ...
    private static function handle_payment_failed($order, $payment) {
        ...
        $order->update_status('failed', __('GembaPay payment failed.', 'gembapay-woocommerce'));
    ...
    private static function handle_payment_expired($order, $payment) {
        ...
        $order->update_status('cancelled', __('GembaPay payment expired.', 'gembapay-woocommerce'));
